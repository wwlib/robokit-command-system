...
class CommandFactory extends events_1.EventEmitter {
    constructor() {
        super();
        this.createCommand = ((data, targetAccountId, createdAtTime, registerCommand = true) => {
            const command = {
                id: uuid_1.v4(),
                targetAccountId,
                type: data.type,
    ...
    createPlayPromptCommand(prompt, targetAccountId, createdAtTime, registerCommand = true) {
        const data = {
    ...
    createPlayMidiNoteCommand(note, channel, volume, startAtTime, targetAccountId, createdAtTime, registerCommand = true) {
        const data = {
            type: RCSCommandType.command,
            name: RCSCommandName.play,
            payload: {
    ...
    createPlayMidiFileCommand(filename, channelsToPlay, startAtTime, targetAccountId, createdAtTime, registerCommand = true) {
        const data = {
            type: RCSCommandType.command,
            name: RCSCommandName.play,
            payload: {
